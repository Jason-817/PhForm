// Floating GIFs
let activeGIFs = [];
function startFloatingGIFs() { for(let i=0;i<15;i++) createFloatingGIF(); }
function stopFloatingGIFs() { activeGIFs.forEach(g=>g.remove()); activeGIFs=[]; }
...
bioOrder.forEach(key => {
    if (bioData[key] !== undefined) {
        content += `${key}: ${bioData[key]}\n`;
    }
});
...
// Matter.js Plinko (unchanged from original)
const {Engine,Render,World,Bodies,Body,Events,Bounds} = Matter;
const engine = Engine.create(), world = engine.world;
engine.gravity.y=0.5;
const render = Render.create({canvas: canvas, engine: engine, options:{width:400,height:600,wireframes:false,background:"#111"}});
...
for(let row=0;row<rows;row++){
    for(let col=0;col<cols;col++){
        ...
        World.add(world,peg); pegs.push(peg);
    }
}
...
function dropCheckbox(){
    const checkbox = Bodies.rectangle(noBox.position.x,50,20,20,{restitution:0.5,friction:0.05,render:{fillStyle:"#00ffff"},custom:{scored:false}});
    Body.setStatic(checkbox,true); World.add(world,checkbox); checkboxes.push(checkbox);
    setTimeout(()=>{Body.setStatic(checkbox,false); Body.setVelocity(checkbox,{x:0,y:2}); Body.setAngularVelocity(checkbox,0.01);},1500);
}
    ...
    event.pairs.forEach(pair=>{
        checkboxes.forEach(cb=>{
            if(pair.bodyA===cb||pair.bodyB===cb){
                ...
                if(pegs.includes(other)){Body.applyForce(cb,cb.position,{x:(Math.random()-0.5)*0.02,y:-0.02-Math.random()*0.01});}
                if(other===noBox){Body.setVelocity(cb,{x:-2,y:-5});}
            }
        });
    });
});
Events.on(engine,'afterUpdate',()=>{
    checkboxes.forEach((cb,i)=>{
        if(!cb.custom.scored&&isOverlapping(cb,yesBox)){
            alert("Checkbox landed in YES!"); startFloatingGIFs(); cb.custom.scored=true; World.remove(world,cb); checkboxes.splice(i,1);
        }
    });
});

document.getElementById("noBtn").addEventListener("click",()=>{canvas.style.display="block"; dropCheckbox();});
document.getElementById("yesBtn").addEventListener("click",()=>{
    const answer=prompt("Are you sure you want to retract your statement?");
    if(answer!==null){alert("Statement retracted."); startFloatingGIFs();}
});